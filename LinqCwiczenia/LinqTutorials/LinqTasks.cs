...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<object> result = Emps.Join(Depts, emps =>  emps.Deptno,  dept => dept.Deptno, (emp, dept) => new {Emp = emp, Dept = dept});
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.GroupBy(e => e.Deptno).Where(e => e.Count() > 1).Select(e => new
            {
                name = e.Key,
        ...
        {
            int result = arr.GroupBy(num => num).Where(e => e.Count() % 2 != 0).Select(e => e.Key).First();
            //result=
            return result;
        }
        ...
        {
            IEnumerable<Dept> result = Depts.Join(Emps.GroupBy(
                emp => emp.Deptno), 
                dept => dept.Deptno, 
                group => group.Key,
        ...
        {
            IEnumerable<Object> result = Emps.Where(emp => emp.Job.Contains("A")).GroupBy(emp => emp.Job).Where(group => group.Count() > 2).OrderByDescending(group => group.Count()).Select(group => new
                                        {
                                            Praca = group.Key,
